...
parser.add_option('-n', '--num_pixels', dest='num_pixels', default=143*2,
                    action='store', type='int', help='number of pixels')
parser.add_option('-s', '--server', dest='server', default='127.0.0.1:7890',
                    action='store', type='string', help='server IP and port')
parser.add_option('-f', '--fps', dest='fps', default=60,
                    action='store', type='int', help='frames per second')

options, args = parser.parse_args()
socket = opc.Client(options.server)

black_white = [(0, 0, 0), (128, 128, 128)]
...
gov = opc.FpsGovernor(options.fps)
while True:
    for pixels, bright in zip(arrays, rgb_bright):
        dim = pixels[0]
        for i in range(options.num_pixels):
            pixels[i] = bright
            pixels[10] = black_white[(frame % 2)]
            pixels[20] = black_white[(frame % 4)//2]
            pixels[30] = black_white[(frame % 10)//5]
            pixels[40] = black_white[(frame % 100)//50]
            pixels[50] = black_white[(frame % 1000)//500]
            socket.put_pixels(pixels)
            gov.end()
            pixels[i] = dim
            frame += 1
            if frame % 100 == 0:
